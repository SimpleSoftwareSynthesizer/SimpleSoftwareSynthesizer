      ...
      selector (0)
{
    ...
    addAndMakeVisible (selector = new AudioDeviceSelectorComponent (deviceManager, 0, 2, 0, 2, true, true, true, false));
    selector->setName (L"selector");
	...
	slider5->setValue(10.0);
	controller = new ADSRController(); 														// Erstelle den Controller für die ADSR-Objekte
    synthAudioSource = new SynthAudioSource (keyboardState, controller);		
    audioSourcePlayer.setSource (synthAudioSource); 										// Füge dem Player eine Audiosource hinzu
	volume = 100;//TODO remove
	
	/*
		deviceManager --> Schnittstelle zum System
	*/
    deviceManager.addAudioCallback (&audioSourcePlayer);									// Der Audio-Callback wird bei auftretenden AudioEreignissen ausgelöst
    deviceManager.addMidiInputCallback (String::empty, &(synthAudioSource->midiCollector));	// Der MIDI-Callback wird bei auftretenden MIDI-Ereignissen ausgelöst.
...
SiSoS_Component::~SiSoS_Component()
{
	...
	audioSourcePlayer.setSource (0);																// Entferne die AudioSource vom Player
    deviceManager.removeMidiInputCallback (String::empty, &(synthAudioSource->midiCollector));		// Entferne den MIDI-Callback
    deviceManager.removeAudioCallback (&audioSourcePlayer);											// Entferne den Audio-Callback
...
void SiSoS_Component::resized()
{
    ...
    selector->setBounds (600, 48, 192, 384);
    //[UserResized]
...
void SiSoS_Component::sliderValueChanged (Slider* sliderThatWasMoved)
{
    ...
    if (sliderThatWasMoved == slider)
    {
		...
		this->controller->setAttackRate(slider->getValue()+1.0);
    ...
    else if (sliderThatWasMoved == slider2)
    {
		...
		this->controller->setDecayRate(slider2->getValue()+1.0);
    ...
    else if (sliderThatWasMoved == slider3)
    {
		...
		this->controller->setSustainRate(slider3->getValue());
    ...
    else if (sliderThatWasMoved == slider4)
    {
		...
		this->controller->setReleaseRate(slider4->getValue());
    ...
    else if (sliderThatWasMoved == slider5)
    {
		...
		audioSourcePlayer.setGain( slider5->getValue()/10);
...
void SiSoS_Component::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    if (buttonThatWasClicked == textButton)
    {
		...
		FileChooser chooser ("Choose file to open", File::getSpecialLocation (File::userHomeDirectory),
                               "*.sisos", true);
		if (chooser.browseForFileToOpen()) {
			File loadFile (chooser.getResult());
			text=loadFile.loadFileAsString();
			...
			//Fill in values
			slider->setValue(String(text.substring(0,pos1)).getDoubleValue());
			slider2->setValue(String(text.substring(pos1,pos2)).getDoubleValue());
			slider3->setValue(String(text.substring(pos2,pos3)).getDoubleValue());
			slider4->setValue(String(text.substring(pos3)).getDoubleValue());
    ...
    else if (buttonThatWasClicked == textButton2)
    {
		...
		//Text to write
		text=String(slider->getValue())+"\n"+String(slider2->getValue())+"\n"+String(slider3->getValue())+"\n"+String(slider4->getValue());

		//Open chooser
		FileChooser chooser("Choose location to save",File::getSpecialLocation (File::userHomeDirectory),
                               "*.sisos", true);
		if (chooser.browseForFileToSave(true)){
			File saveFile (chooser.getResult());
			saveFile=saveFile.withFileExtension("sisos");
			saveFile.replaceWithText(text);
...
<JUCER_COMPONENT documentType="Component" className="SiSoS_Component" componentName=""
                 ...
                 fixedSize="1" initialWidth="800" initialHeight="500">
  ...
  <GENERICCOMPONENT name="new component" id="b54de0be00f9dd3c" memberName="component"
                    ...
                    params=""/>
  <GENERICCOMPONENT name="selector" id="e9893bc00b725af8" memberName="selector" virtualName=""
                    explicitFocusOrder="0" pos="600 48 192 384" class="AudioDeviceSelectorComponent"
